...
BEGIN
    ...
    IF (NEW.spec).type = 'kubernetes' THEN
        ...
        IF (NEW.spec).config->'router'->>'replicas' IS NULL OR trim((NEW.spec).config->'router'->>'replicas') = '' THEN
            RAISE sqlstate 'PGRST'
        ...
        -- Check that replicas is an integer >= 1
        BEGIN
            IF (NEW.spec).config->'router'->>'replicas'::INTEGER < 1 THEN
                RAISE sqlstate 'PGRST'
        ...
        IF (NEW.spec).config->'router'->>'resources' IS NULL THEN
            RAISE sqlstate 'PGRST'
                USING message = '{"code": "10024","message": "router.resources is required for Kubernetes clusters","hint": "Provide router.resources in config"}',
                detail = '{"status": 400, "headers": {"X-Powered-By": "Neutree"}}';
        END IF;
