...
CREATE POLICY api_key_projects_insert ON api.api_key_projects FOR INSERT
    WITH CHECK (user_id = auth.uid());
CREATE POLICY api_key_projects_update ON api.api_key_projects FOR UPDATE
    USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE POLICY api_key_projects_delete ON api.api_key_projects FOR DELETE
    USING (user_id = auth.uid());
