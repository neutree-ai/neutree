...
BEGIN
    ...
    FROM api.api_keys moving JOIN api.api_keys existing
      ON existing.project_id = p_project_id AND (existing.metadata).name = (moving.metadata).name AND existing.id <> moving.id
    WHERE moving.id = ANY(p_api_key_ids);
    IF v_conflicts IS NOT NULL THEN RAISE EXCEPTION 'API key name conflicts: %', v_conflicts USING ERRCODE = '23505', DETAIL = v_conflicts; END IF;
    INSERT INTO api.api_key_project_history (api_key_id, from_project_id, to_project_id, moved_by)
    SELECT id, project_id, p_project_id, auth.uid() FROM api.api_keys WHERE id = ANY(p_api_key_ids) AND project_id <> p_project_id;
