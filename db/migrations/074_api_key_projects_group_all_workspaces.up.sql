-- group_projects gains two capabilities needed by the all-workspaces list
-- view:
--   1) p_workspace = NULL returns every Project the caller can read, so the
--      frontend gets counts + current-cycle usage for ALL workspaces in one
--      batched call (no per-workspace fan-out).
--   2) the Project's workspace is returned so the UI can scope create /
--      migrate / view actions without a second query.
...
BEGIN
    IF p_workspace IS NOT NULL
       AND NOT api.has_permission(auth.uid(), 'workspace:read', p_workspace) THEN
    ...
    WHERE (p_workspace IS NULL OR p.workspace = p_workspace)
      AND (p_workspace IS NOT NULL
           OR api.has_permission(auth.uid(), 'workspace:read', p.workspace))
