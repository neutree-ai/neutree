...
BEGIN
    ...
    IF v_key.project_id <> p_project_id THEN
        IF EXISTS (
            SELECT 1 FROM api.api_keys existing
            WHERE existing.project_id = p_project_id
              AND (existing.metadata).name = (v_key.metadata).name
              AND existing.id <> p_api_key_id
              AND (existing.metadata).deletion_timestamp IS NULL
        ) THEN
            RAISE EXCEPTION 'API key name conflicts: %', (v_key.metadata).name
                USING ERRCODE = '23505', DETAIL = (v_key.metadata).name;
        END IF;

        INSERT INTO api.api_key_project_history
            (api_key_id, from_project_id, to_project_id, moved_by)
        VALUES (p_api_key_id, v_key.project_id, p_project_id, auth.uid());
