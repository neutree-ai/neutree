-- Restore original record_api_usage function (from migration 018)
...
-- Restore original aggregate_usage_records function (from migration 019)
...
-- Restore original get_usage_by_dimension function (from migration 019)
...
-- Drop detailed_dimensional_usage attribute from api_daily_usage_spec
ALTER TYPE api.api_daily_usage_spec DROP ATTRIBUTE IF EXISTS detailed_dimensional_usage;

-- Drop new columns from api_usage_records
ALTER TABLE api.api_usage_records DROP COLUMN IF EXISTS endpoint_type;
ALTER TABLE api.api_usage_records DROP COLUMN IF EXISTS model_name;
ALTER TABLE api.api_usage_records DROP COLUMN IF EXISTS prompt_tokens;
ALTER TABLE api.api_usage_records DROP COLUMN IF EXISTS completion_tokens;
