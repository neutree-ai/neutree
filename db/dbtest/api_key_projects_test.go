...
func TestAPIKeyProjects(t *testing.T) {
	...
	WithUserContext(t, db, user.ID, func(tx *sql.Tx) {
		...
		if _, err := tx.Exec(`SELECT api.move_api_keys_to_project(ARRAY[$1::uuid],$2)`, key1, target); err == nil || !strings.Contains(err.Error(), "conflicts") {
		...
		if total != 4 {
			t.Fatalf("all-workspace total=%d want 4", total)
		}
