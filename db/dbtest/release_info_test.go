...
func TestReleaseInfoMigrationRoundTripCreatesOnlyFinalSchema(t *testing.T) {
	upMigration, err := os.ReadFile("../migrations/089_release_info_cluster_profiles.up.sql")
	if err != nil {
		t.Fatalf("read forward migration: %v", err)
	}

	downMigration, err := os.ReadFile("../migrations/089_release_info_cluster_profiles.down.sql")
	if err != nil {
