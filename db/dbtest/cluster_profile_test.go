...
func TestClusterProfileIsGlobalInternalState(t *testing.T) {
	...
	if _, err = serviceTx.ExecContext(ctx, `
		...
		VALUES (
			...
			'{"cluster_type":"ssh","components":{"ray_runtime":{"image":"neutree/neutree-serve","tag":"v1.2.0-rc.1"}}}'::jsonb
		)
	`, profileName); err != nil {
	...
	kubernetesTx, err := adminDB.BeginTx(ctx, nil)
	if err != nil {
		t.Fatalf("begin kubernetes service role transaction: %v", err)
	}
	defer func() {
		_ = kubernetesTx.Rollback()
	}()
	if _, err = kubernetesTx.ExecContext(ctx, "SET LOCAL ROLE service_role"); err != nil {
		t.Fatalf("set kubernetes service role: %v", err)
	}
	if _, err = kubernetesTx.ExecContext(ctx, `
		INSERT INTO api.cluster_profiles (api_version, kind, metadata, spec)
		VALUES (
			'v1',
			'ClusterProfile',
			ROW($1, NULL, NULL, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '{}'::json, '{}'::json)::api.metadata,
			'{"cluster_type":"kubernetes","components":{"kubernetes_runtime":{"image":"neutree/neutree-runtime","tag":"v1.2.0-rc.1"}}}'::jsonb
		)
	`, profileName); err != nil {
		t.Fatalf("expected same version for kubernetes to be accepted: %v", err)
	}
	if err = kubernetesTx.Commit(); err != nil {
		t.Fatalf("commit kubernetes profile: %v", err)
	}
	...
	_, err = duplicateTx.ExecContext(ctx, `
		...
		VALUES (
			...
			'ClusterProfile',
			ROW($1, NULL, NULL, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '{}'::json, '{}'::json)::api.metadata,
			'{"cluster_type":"ssh","components":{}}'::jsonb
		)
	`, profileName)
	if err == nil {
		t.Fatal("expected duplicate cluster profile version/type to be rejected")
	}
	...
	invalidTypeTx, err := adminDB.BeginTx(ctx, nil)
	if err != nil {
		t.Fatalf("begin invalid type transaction: %v", err)
	}
	defer func() {
		_ = invalidTypeTx.Rollback()
	}()
	if _, err = invalidTypeTx.ExecContext(ctx, "SET LOCAL ROLE service_role"); err != nil {
		t.Fatalf("set invalid type service role: %v", err)
	}
	_, err = invalidTypeTx.ExecContext(ctx, `
		INSERT INTO api.cluster_profiles (api_version, kind, metadata, spec)
		VALUES (
			'v1',
			'ClusterProfile',
			ROW('v1.2.0-rc.4', NULL, NULL, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '{}'::json, '{}'::json)::api.metadata,
			'{"components":{}}'::jsonb
		)
	`)
	if err == nil {
		t.Fatal("expected missing cluster profile type to be rejected")
	}
}
