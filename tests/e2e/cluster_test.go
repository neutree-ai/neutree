...
func testImageRegistry() string {
	if r := os.Getenv("E2E_IMAGE_REGISTRY"); r != "" {
		return r
	}
	return "e2e-image-registry-" + runID
}

func testClusterVersion() string {
	if v := os.Getenv("E2E_CLUSTER_VERSION"); v != "" {
		return v
	}
	return "v1.0.0"
}
...
func requireSSHEnv() (headIP, workerIPs, sshUser, sshPrivateKey string) {
	headIP = os.Getenv("E2E_SSH_HEAD_IP")
	if headIP == "" {
		Skip("E2E_SSH_HEAD_IP not set, skipping SSH cluster tests")
	}

	workerIPs = os.Getenv("E2E_SSH_WORKER_IPS")
...
// requireK8sEnv returns the base64-encoded kubeconfig from env.
...
var _ = Describe("Cluster Status", Ordered, Label("cluster"), func() {
	...
	Describe("SSH Cluster Lifecycle", Ordered, Label("ssh"), func() {
		...
		It("should not trigger Updating on credential change", func() {
			By("Recording current state")
			r := ClusterH.Get(clusterName)
			ExpectSuccess(r)
			c := parseClusterJSON(r.Stdout)
			...
			By("Applying with a different ssh_private_key (same spec as previous)")
			...
			By("Waiting briefly for controller to reconcile")
			time.Sleep(10 * time.Second)

			By("Verifying phase is still Running and hash unchanged")
			r = ClusterH.Get(clusterName)
			ExpectSuccess(r)
			c = parseClusterJSON(r.Stdout)
			Expect(c.Status.Phase).To(Equal("Running"))
			Expect(c.Status.ObservedSpecHash).To(Equal(oldHash),
				"spec hash should not change on credential-only update")
		})
	...
	Describe("K8s Cluster Lifecycle", Ordered, Label("k8s"), func() {
		...
		It("should not trigger Updating on kubeconfig change", func() {
			By("Recording current state")
			r := ClusterH.Get(clusterName)
			ExpectSuccess(r)
			c := parseClusterJSON(r.Stdout)
			...
			By("Applying with a modified kubeconfig (same spec as previous)")
			// Decode the original base64, append a comment, re-encode.
			origBytes, err := base64.StdEncoding.DecodeString(kubeconfig)
	...
	Describe("Initializing with bad dependency", Label("status"), func() {
		...
		It("should stay Initializing when image registry is invalid", func() {
			...
			yaml := renderSSHClusterYAML(map[string]string{
				...
				"ssh_user":        sshUser,
				"ssh_private_key": sshPrivateKey,
				"image_registry":  "non-existent-registry-" + runID,
			})
			r := ClusterH.Apply(yaml)
			ExpectSuccess(r)
