...
# gemma4: module paths unchanged from v0.17.1
...
class Backend:
    def __init__(self,
        ...
        # gemma4: new FrontendArgs params now accepted by OpenAIServingChat
        ...
        # gemma4: --task removed, replaced by --runner/--convert (both default "auto").
    ...
    def _ensure_serving_render(self, models):
        """Create OpenAIServingRender for the gemma4 rendering pipeline."""
    ...
    async def _ensure_score(self):
        if self.openai_serving_score is None:
            self._ensure_model_config()
            models = await self._ensure_models()
            # gemma4: use_gpu_for_pooling_score removed; score_template/log_error_stack added
