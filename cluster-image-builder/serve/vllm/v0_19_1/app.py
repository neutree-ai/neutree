...
# v0.19.0: new render serving for disaggregated chat preprocessing
...
class Backend:
    def __init__(self,
        ...
        # v0.19.0: use_gpu_for_pooling_score removed from ServingScores
    ...
    async def _ensure_chat(self):
        if self.openai_serving_chat is None:
            ...
            # v0.19.1: OpenAIServingRender adds reasoning_parser param
    ...
    async def _ensure_score(self):
        if self.openai_serving_score is None:
            self._ensure_model_config()
            models = await self._ensure_models()
            # v0.19.0: use_gpu_for_pooling_score removed; score_template added
