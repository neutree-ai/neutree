...
End-to-end runtime layout (1 replica = 1 PDCollocatedBackend):
    ...
    PDRouter (FastAPI, 0.1 CPU, 0 GPU)
        │  Selects RoleGroup + prefill/decode ranks; ObserverRouter dispatches replica
        ▼
    PDCollocatedBackend  (0.1 CPU, 0 GPU; owns PG + protocol conversion)
        ├── placement_group(STRICT_PACK, x + y bundles)
        ├── PrefillActor[0..x-1]     (extends standard Backend)
        │       bundle 0..x-1
        │       runtime_env from Role.Env + portalloc port (user wins)
        └── DecodeActor[0..y-1]      (extends standard Backend)
                bundle x..x+y-1

Per-request flow inside PDCollocatedBackend.generate:
    1. Receive prefill_index/decode_index selected by PDRouter.
...
# Reuse the standard Backend implementation: model download, AsyncLLM
...
# with @ray.remote.
from serve.vllm.v0_20_0.app import _Backend as Backend
...
def _vllm_port_env(pd_config: Dict[str, Any], role_group_index: int,
    ...
    """Engine-side named-port convention.

    Reads pd_config["ports"][role_group_index][role_name][rank] =
    {"http": ..., "side_channel": ...} and maps side_channel to
    ...
    port = _port_value(slot, "side_channel", role_group_index, role_name, rank)
    return {
        "VLLM_NIXL_SIDE_CHANNEL_HOST": "0.0.0.0",
        "VLLM_NIXL_SIDE_CHANNEL_PORT": str(port),
    }
...
def _port_value(slot: Any, purpose: str, role_group_index: int, role: str, rank: int) -> int:
    ...
    if port <= 0:
        raise RuntimeError(
            f"empty port slot for role_group={role_group_index} role={role} "
            f"rank={rank} purpose={purpose}"
        )
    return port
...
def _role_actor_name(pd_config: Dict[str, Any], role_group_key: str,
    ...
    if not role_group_key:
        raise RuntimeError(
            "PD role actor naming requires a non-empty Serve replica key"
        )
...
def _role_resources_to_ray(role: Dict[str, Any]) -> Dict[str, Any]:
    ...
    helper is almost a direct passthrough. CPU/GPU default to Ray's normal
    actor defaults so the placement-group bundle includes the same implicit
    resources the actor would otherwise request.
    """
...
class PrefillActor(Backend):
    ...
    # Sync __init__ to mirror parent _Backend (see app.py: async __init__
    # is unreliable across Ray Serve decorator paths; do_log_stats pump
    # is now scheduled via create_task on the actor's event loop, with
    # a lazy ensure in _ensure_models as fallback).
    def __init__(self, *,
...
class DecodeActor(Backend):
    ...
    # See PrefillActor.__init__ — same sync rationale.
    def __init__(self, *,
                 model_args: Dict[str, Any],
                 engine_kwargs: Dict[str, Any],
                 kv_extra: Dict[str, Any]):
...
class PDCollocatedBackend:
    ...
    via STRICT_PACK PG. PDRouter selects the RoleGroup and local
    ...
    def __init__(self, *,
        ...
        # model in the background. Same shape as the standard Backend
    ...
    async def generate(
        ...
        """Chat-completion protocol conversion for the router-selected P/D pair.
        ...
        prefill_payload = dict(clean_payload)
        ...
        log.info(
            "[pd_generate][pair] req=%s prefill_rank=%d decode_rank=%d "
        ...
        if isinstance(prefill_resp, dict) and "error" in prefill_resp:
            log.warning(
                "[pd_generate][prefill_error] req=%s err=%s", req_id, prefill_resp.get("error"),
            )
            return prefill_resp
        ...
        log.info(
            "[pd_generate][prefill_done] req=%s resp_type=%s kv_source=%s "
        ...
        decode_payload = dict(clean_payload)
        ...
        log.info(
            "[pd_generate][decode_dispatch] req=%s kv_injected=%s stream=%s "
        ...
        if is_stream:
            ...
            # the decode actor's streaming chunks back to PDRouter, which
            # is wrapping generate with options(stream=True). Returning a
    ...
    async def _stream_decode(self, decode_handle, decode_payload, req_id):
        ...
        try:
            ...
            log.warning(
                "[pd_generate][stream_error] req=%s err=%s", req_id, exc,
            )
            raise
    ...
    async def show_available_models(self):
        ...
        shape so the JSONResponse wrapper in PDRouter passes it through
        unchanged.
        """
        return {
            "object": "list",
