...
# v0.18.1: module paths unchanged from v0.17.1
...
# v0.18.1: new rendering pipeline for OpenAIServingChat
...
class Backend:
    def __init__(self,
        ...
        # v0.17.1 FrontendArgs with functional impact
        self.default_chat_template_kwargs = engine_kwargs.pop("default_chat_template_kwargs", None)

        # v0.18.1: new FrontendArgs params now accepted by OpenAIServingChat
        ...
        # v0.18.1: --task removed, replaced by --runner/--convert (both default "auto").
        ...
        # Store engine_args for creating VllmConfig later (needed by rendering pipeline)
    ...
    def _ensure_serving_render(self, models):
        """Create OpenAIServingRender for the v0.18.1 rendering pipeline."""
        if self.openai_serving_render is None:
            ...
            io_processor = get_io_processor(
                vllm_config, renderer,
                vllm_config.model_config.io_processor_plugin
            )
    ...
    async def _ensure_chat(self):
        if self.openai_serving_chat is None:
            ...
            render = self._ensure_serving_render(models)
    ...
    async def _ensure_score(self):
        if self.openai_serving_score is None:
            self._ensure_model_config()
            models = await self._ensure_models()
            # v0.18.1: use_gpu_for_pooling_score removed; score_template/log_error_stack added
    ...
    async def generate_embeddings(self, payload: Any):
        ...
        # v0.18.1: ServingEmbedding uses __call__ instead of create_embedding
...
class Controller:
    ...
    async def embeddings(self, request: Request):
        ...
        if isinstance(result, ErrorResponse):
            return JSONResponse(content=result.model_dump(), status_code=result.code)
        # v0.18.1: ServingEmbedding.__call__ returns a Response directly
