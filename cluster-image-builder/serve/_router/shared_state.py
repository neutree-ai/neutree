"""Module-level _SHARED view for PDRouter.

Architecture invariant:
    Inside a single Python process (the PDRouter replica), a module-level
    ...
    handlers. Multiple router processes (num_replicas > 1) each maintain their
    own _SHARED.
...
Data model:
    ...
    actor_topology: {replica_id_str -> ActorTopology}
        Populated lazily by PDRouter when it first needs to know which node
...
Event hooks:
    ...
    PDRouter uses replica_added to schedule an immediate get_actor_topology
...
# land in the replica's stdout. The "pd_router.shared" custom name had
...
class RouteUnit:
    ...
    For Ray, domain is the canonical Ray Serve replica id. The transport target
    is the direct-dispatched Serve handle, so url is intentionally empty.
    """
    domain: str
    role: str
    rank: int
    ...
    def unit_id(self) -> str:
        ...
        return f"{self.domain}:{self.role}:{self.rank}"
...
class PDTopologyUnit:
    ...
    Keep this shape aligned with the K8s router topology contract consumed by
    production-stack service discovery:
        {"role": "prefill" | "decode", "rank": <non-negative int>}
    """
    role: str
    rank: int
...
class ActorTopology:
    ...
    group_id          — P/D scheduling domain. For Ray this is the canonical
                        Ray Serve ReplicaID, matching K8s' group_id contract.
    units             — minimal schedulable topology, aligned with K8s
                        /v1/pd/topology: one {role, rank} per P/D unit.
    ...
    """
    group_id: str = ""
    units: List[PDTopologyUnit] = field(default_factory=list)
    replica_id: str = ""
...
class _Shared:
    ...
    def upsert_topology(self, replica_id: str, topo: ActorTopology) -> None:
        """Called by PDRouter after pulling topology from a backend replica."""
        with self._lock:
...
# Module-level singleton. All ObserverRouter instances + the PDRouter request
