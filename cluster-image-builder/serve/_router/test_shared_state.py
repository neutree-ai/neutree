...
def test_topology_updated_event_carries_full_topology():
    ...
    s.upsert_topology(
        ...
        ActorTopology(
            group_id="r0",
            units=[
                PDTopologyUnit(role="prefill", rank=0),
                PDTopologyUnit(role="prefill", rank=1),
                PDTopologyUnit(role="decode", rank=0),
            ],
            replica_id="r0",
            prefills=[
...
def test_actor_topology_uses_k8s_compatible_group_units():
    topo = ActorTopology(
        group_id="replica-0",
        units=[
            PDTopologyUnit(role="prefill", rank=0),
    ...
    assert topo.group_id == "replica-0"
    assert [(unit.role, unit.rank) for unit in topo.units] == [
        ("prefill", 0),
        ("decode", 0),
    ]
    assert not hasattr(topo.units[0], "domain")


def test_route_unit_id_uses_domain_role_rank_and_optional_url():
    ray_unit = RouteUnit(domain="replica-0", role="decode", rank=1)
    http_unit = RouteUnit(
        domain="pod-0",
        role="prefill",
        rank=0,
    ...
    assert ray_unit.unit_id == "replica-0:decode:1"
    assert http_unit.unit_id == "pod-0:prefill:0:http://10.0.0.1:9000"
...
def test_replica_dispatch_uses_bare_multiplexed_model_id():
    """Direct dispatch treats multiplexed_model_id as a bare ReplicaID.

    Validates that:
      - non-empty target -> direct ReplicaID target
    ...
    assert classify("abc") == ("direct", "abc")
    assert classify("default#ep#PDCB#xyz") == (
