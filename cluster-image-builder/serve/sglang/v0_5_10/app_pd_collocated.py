...
class PDCollocatedBackend:
    ...
    async def pd_chat(
        ...
        if stream:
            return self._stream_pd_chat(prefill_handle, decode_handle, prefill_payload, decode_payload, req_id)

        prefill_resp, decode_resp = await asyncio_gather(
            prefill_handle.chat_completion.remote(prefill_payload),
            decode_handle.chat_completion.remote(decode_payload),
        )
        if _is_error_payload(prefill_resp):
            log.warning("[sglang_pd_chat][prefill_error] req=%s resp=%s", req_id, prefill_resp)
            return {"error": prefill_resp}
        if _is_error_payload(decode_resp):
            log.warning("[sglang_pd_chat][decode_error] req=%s resp=%s", req_id, decode_resp)
    ...
    async def _drain_actor_stream(self, actor_stream, req_id: str, role: str) -> None:
        try:
            ...
            log.warning("[sglang_pd_chat][%s_stream_error] req=%s err=%s", role, req_id, exc)
            raise

    async def _stream_pd_chat(
        self,
        prefill_handle,
        decode_handle,
