...
Notes:
    * No ``_normalize_tool_calls`` shim: the Kong gateway plugin
      (``gateway/kong/plugins/neutree-ai-gateway/handler.lua``) added an
      ``is_table()`` guard in #368 (commit cccc57d8) so null ``tool_calls``
      is handled at the gateway boundary; doing it again here is dead code.
    * No SGLang signal-handler monkey-patch: ``v0.5.10`` upstream already
      contains the ``main_thread()`` guard from PR #18752, so Ray Serve
      worker threads can construct ``Engine`` without hitting
...
class Backend:
    def __init__(
        ...
        **engine_kwargs,
        ...
        # SGLang exposes an `is_embedding` flag for embedding/reranker models;
        # map it from the registry-level model_task so users don't have to set
        # it twice. Rerank shares the embedding code path in SGLang.
        if model_task in ("text-embedding", "text-rerank"):
