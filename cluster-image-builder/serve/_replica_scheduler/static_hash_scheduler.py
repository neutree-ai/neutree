...
class StaticHashReplicaScheduler(RequestRouter):
    ...
    async def choose_replicas(
        ...
        # Use the hash to determine the starting index
        start_idx = int(payload_hash, 16) % len(candidate_replicas)

        # Build priority-ordered list starting from the hashed index
        result: List[List[RunningReplica]] = []
        for i in range(len(candidate_replicas)):
            idx = (start_idx + i) % len(candidate_replicas)
            result.append([candidate_replicas[idx]])

        selected_replica = candidate_replicas[start_idx]
        logger.info(
            f"StaticHashScheduler: Payload hash={payload_hash[:8]}..., "
            f"Selected Replica={selected_replica.replica_id} "
            f"(index {start_idx} of {len(candidate_replicas)})"
        )
