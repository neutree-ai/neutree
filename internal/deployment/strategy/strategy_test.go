...
func TestGet_DefaultAndLegacyAlias(t *testing.T) {
	s, err := Get("standard")
	if err != nil {
	...
	legacy, err := Get("monolithic")
	if err != nil {
		t.Fatalf("Get(monolithic): %v", err)
	}
	if legacy != s {
		t.Fatalf("monolithic alias should resolve to standard")
	}
}
...
func TestPD_Validate_SameHost1P1D(t *testing.T) {
	s, _ := Get("pd")
