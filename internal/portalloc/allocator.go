...
// AllocateForPDConfig: idempotent reservation of every port slot the PD config
// asks for. Algorithm:
...
//     unused port for each (RoleGroup x role x rank x purpose) slot the
//     config still needs.
...
func (a *allocator) AllocateForPDConfig(
	ctx context.Context,
	cluster *v1.Cluster,
	endpointID int,
	...
	// `required` already walks role_group_index -> router -> Group.Roles
	// (declaration order) -> rank -> purpose. We keep that natural order so
	// allocation maps to the config top-to-bottom deterministically.
...
func buildRequiredSlots(cfg *pdconfig.PDConfig) ([]slotKey, error) {
	...
	for replica := 0; replica < cfg.NumReplicas; replica++ {
		out = append(out, slotKey{
			...
			Role:           pdconfig.PortRoleRouter,
			Rank:           0,
			Purpose:        pdconfig.PortPurposeHTTP,
		})
		...
		for _, role := range cfg.Group.Roles {
			...
			purposes, err := portPurposes(role.PortsPerRank)
			if err != nil {
				return nil, fmt.Errorf(
					"portalloc: role %s requests %d ports per rank: %w",
					role.Name, role.PortsPerRank, err,
				)
			}
...
func portPurposes(portsPerRank int) ([]string, error) {
	switch portsPerRank {
	case 1:
		return []string{pdconfig.PortPurposeSideChannel}, nil
	case 2:
		return []string{pdconfig.PortPurposeHTTP, pdconfig.PortPurposeSideChannel}, nil
	default:
		return nil, fmt.Errorf("strategy=pd supports one legacy side-channel port or http+side_channel")
	}
}
...
func isCompleteFor(required []slotKey, existing []Allocation) bool {
	...
	for _, a := range existing {
		have[slotKey{a.RoleGroupIndex, a.Role, a.Rank, a.Purpose}] = struct{}{}
	}
...
// config shape: Ports[role_group_index][role][rank][purpose] = port.
//
...
func materializePorts(cfg *pdconfig.PDConfig, allocations []Allocation) []pdconfig.ReplicaPortMap {
	...
	// Pre-size purpose maps so renderer and engine-side indexing work even
	// before any allocation row lands for that slot.
	for roleGroupIdx := 0; roleGroupIdx < cfg.NumReplicas; roleGroupIdx++ {
		ports[roleGroupIdx][pdconfig.PortRoleRouter] = []pdconfig.RankPortMap{
			{pdconfig.PortPurposeHTTP: 0},
		}
		...
		for _, role := range cfg.Group.Roles {
			...
			ranks := make([]pdconfig.RankPortMap, role.Instances)

			for rank := 0; rank < role.Instances; rank++ {
				ranks[rank] = make(pdconfig.RankPortMap, len(purposes))
				for _, purpose := range purposes {
					ranks[rank][purpose] = 0
				}
			}
	...
	for _, a := range allocations {
		...
		if role[a.Rank] == nil {
			role[a.Rank] = pdconfig.RankPortMap{}
		}

		role[a.Rank][a.Purpose] = a.Port
	}
