...
func (h *HAMiComponent) planNodeScope(ctx context.Context, nodes []corev1.Node, enabled bool) (NodeScopePlan, error) {
	...
	plan.SupportedResources = config.SupportedResources
...
func (h *HAMiComponent) resolveVirtualizationConfigs(
	...
	configs := make([]*accelerator.VirtualizationConfig, 0)
	...
	for _, acceleratorType := range h.pluginProvider.SupportPlugins() {
		...
		configProvider, ok := acceleratorPlugin.(accelerator.ClusterVirtualizationConfigProvider)
		if !ok {
...
func mergeVirtualizationConfigs(configs []*accelerator.VirtualizationConfig) (*accelerator.VirtualizationConfig, error) {
	owners := make([]*accelerator.VirtualizationConfig, 0, len(configs))
...
func supportedNodeScopeLabels(configs []*accelerator.VirtualizationConfig) []NodeScopeLabel {
...
func virtualizationConfigBlocked(config *accelerator.VirtualizationConfig) error {
	if !config.Supported {
...
func nodeScopeLabelFromPlugin(label accelerator.VirtualizationNodeScopeLabel) NodeScopeLabel {
