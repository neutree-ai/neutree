...
func TestValidateAcceleratorVirtualizationConfigPatch(t *testing.T) {
	tests := []struct {
		...
		{
			name: "rejects customizing admission webhook namespace selector",
			configPatch: map[string]interface{}{
				"scheduler": map[string]interface{}{
					"admissionWebhook": map[string]interface{}{
						"namespaceSelector": map[string]interface{}{
							"matchLabels": map[string]interface{}{
								"kubernetes.io/metadata.name": "neutree-system",
							},
						},
					},
				},
		...
		{
			name: "rejects customizing admission webhook failure policy",
			configPatch: map[string]interface{}{
				"scheduler": map[string]interface{}{
					"admissionWebhook": map[string]interface{}{
						"failurePolicy": "Fail",
