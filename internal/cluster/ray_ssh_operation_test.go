...
func TestGenerateRayClusterConfig(t *testing.T) {
	...
	commonArgs := fmt.Sprintf(`--system-config={"prestart_worker_first_driver":false,"num_workers_soft_limit":0} `+
	...
	tests := []struct {
		...
		},
		{
			...
			expectedConfig: func() *v1.RayClusterConfig {
				...
				newStaticWorkerLabel := fmt.Sprintf(`--labels='{"%s":"%s","%s":"%s"}'`,
					v1.NeutreeNodeProvisionTypeLabel, v1.StaticNodeProvisionType,
					v1.NeutreeServingVersionLabel, newVersion)
				newCommonArgs := fmt.Sprintf(`--system-config={"prestart_worker_first_driver":false,"num_workers_soft_limit":0} `+
					`--disable-usage-stats --node-manager-port=8077 --dashboard-agent-listen-port=52365 `+
					"--min-worker-port=10002 --max-worker-port=20000 "+
		...
		{
			name: "success - version v1.0.1 without registry auth does not add docker login to start commands",
			cluster: &v1.Cluster{
				Metadata: &v1.Metadata{Name: "test-cluster"},
				Spec: &v1.ClusterSpec{
					Version: "v1.0.1",
