...
func (c *NativeKubernetesClusterReconciler) reconcileModelCache(reconcileCtx *ReconcileContext) error {
	...
	if err != nil {
		return errors.Wrapf(err, "failed to recocile model cache resource")
	}
...
func (c *NativeKubernetesClusterReconciler) reconcileModelCacheStatus(
	...
	for _, cache := range reconcileCtx.kubernetesClusterConfig.ModelCaches {
		...
		if cache.PVC != nil {
			...
			if err != nil {
				errs = append(errs, errors.Wrapf(err, "failed to get model cache pvc %s status", util.PvcName(cache)))
			}

			if !ready {
				errs = append(errs, errors.Errorf("model cache pvc %s is not ready to use now", util.PvcName(cache)))
			}
		}
	}
...
func pvcStatus(ctx context.Context, ctrClient client.Client, pvc *corev1.PersistentVolumeClaim) (bool, error) {
	...
	if err != nil {
		return false, errors.Wrap(err, "failed to get relate pv")
	}
