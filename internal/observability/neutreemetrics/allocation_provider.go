...
type AllocationProvider interface {
	Allocations(ctx context.Context, snapshot *NodeSnapshot) ([]v1.StaticNodeAllocationStatus, error)
}

type AllocationProviderFunc func(ctx context.Context, snapshot *NodeSnapshot) ([]v1.StaticNodeAllocationStatus, error)

func (f AllocationProviderFunc) Allocations(
	ctx context.Context,
...
type PodResourceLister interface {
	ListPodResources(ctx context.Context) ([]PodResource, error)
}

type PodResourceListerFunc func(ctx context.Context) ([]PodResource, error)

func (f PodResourceListerFunc) ListPodResources(ctx context.Context) ([]PodResource, error) {
	return f(ctx)
}
...
func (p RayServeAllocationProvider) rayNodeID(service dashboard.DashboardService) (string, error) {
	...
	for _, node := range nodes {
		if node.IP == p.NodeIP && node.Raylet.State == v1.AliveNodeState {
...
func containerDeviceRefs(containers []ContainerDevices) []string {
	refs := make([]string, 0)
...
func endpointAllocationsFromStaticNodeAllocations(
	...
	result := make([]EndpointAllocation, 0, len(allocations))
	...
	for _, allocation := range allocations {
		...
		result = append(result, EndpointAllocation{
