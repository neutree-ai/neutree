...
func explicitEndpointReplicaGPUUsageUtilizationUUIDs(usages []model.EndpointReplicaGPUUsage) map[string]struct{} {
	result := map[string]struct{}{}

	for _, usage := range usages {
		if usage.GPUUUID == "" || usage.UtilizationRatio == nil {
			continue
		}
...
func NormalizeEndpointReplicaGPUUsageFromDCGMSamples(
	...
	explicitMemoryUsedBytes := explicitEndpointReplicaGPUUsageMemoryUsedBytes(labels, explicitUsages)
	explicitUtilizationUUIDs := explicitEndpointReplicaGPUUsageUtilizationUUIDs(explicitUsages)
	uniqueAllocations := uniqueEndpointAllocationsByGPUUUID(allocations)
	...
	for uuid, allocation := range uniqueAllocations {
		...
		if _, explicit := explicitEndpointReplicaGPUUsageMemoryUsedBytesForAllocation(
