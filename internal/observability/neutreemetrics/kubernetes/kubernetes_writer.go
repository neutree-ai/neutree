...
func (w *AnnotationWriter) writeNodeDevices(
	ctx context.Context,
...
func (w *AnnotationWriter) writePodAllocations(
	ctx context.Context,
	allocations []v1.StaticNodeAllocationStatus,
) error {
	podList := &corev1.PodList{}
	if err := w.Client.List(ctx, podList, client.MatchingFields{"spec.nodeName": w.NodeName}); err != nil {
...
func allocationMatchesPod(allocation v1.StaticNodeAllocationStatus, pod corev1.Pod) bool {
	...
	for _, candidate := range candidates {
		...
		if candidate == namespacedName || candidate == string(pod.UID) {
			return true
		}
	}
