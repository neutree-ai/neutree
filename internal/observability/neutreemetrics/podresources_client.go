...
func (l KubeletPodResourceLister) ListPodResources(ctx context.Context) ([]PodResource, error) {
...
func podResourcesFromKubelet(input []*podresourcesv1.PodResources) []PodResource {
	result := make([]PodResource, 0, len(input))
	...
	for _, pod := range input {
		...
		containers := make([]ContainerDevices, 0, len(pod.GetContainers()))
		...
		for _, container := range pod.GetContainers() {
			...
			for _, device := range container.GetDevices() {
				...
				containers = append(containers, ContainerDevices{
