...
func TestCGroupFSUsageReaderPrefersCGroupV1ControllersInHybridHierarchy(t *testing.T) {
	root := t.TempDir()
	require.NoError(t, os.MkdirAll(filepath.Join(root, "proc", "1234"), 0o755))
	require.NoError(t, os.MkdirAll(filepath.Join(root, "sys/fs/cgroup/cpu,cpuacct/docker/abc"), 0o755))
	require.NoError(t, os.MkdirAll(filepath.Join(root, "sys/fs/cgroup/memory/docker/abc"), 0o755))
	require.NoError(t, os.WriteFile(
		filepath.Join(root, "proc", "1234", "cgroup"),
	...
	require.NoError(t, os.WriteFile(
		filepath.Join(root, "sys/fs/cgroup/cpu,cpuacct/docker/abc", "cpuacct.usage"),
		[]byte("12500000000\n"),
		0o644,
	))
	require.NoError(t, os.WriteFile(
		filepath.Join(root, "sys/fs/cgroup/cpu,cpuacct/docker/abc", "cpu.cfs_quota_us"),
		[]byte("-1\n"),
		0o644,
	))
	require.NoError(t, os.WriteFile(
		filepath.Join(root, "sys/fs/cgroup/cpu,cpuacct/docker/abc", "cpu.cfs_period_us"),
		[]byte("100000\n"),
		0o644,
	))
	require.NoError(t, os.WriteFile(
		filepath.Join(root, "sys/fs/cgroup/memory/docker/abc", "memory.usage_in_bytes"),
		[]byte("4096\n"),
		0o644,
	))
	require.NoError(t, os.WriteFile(
		filepath.Join(root, "sys/fs/cgroup/memory/docker/abc", "memory.stat"),
		[]byte("total_inactive_file 1024\n"),
		0o644,
	))
	require.NoError(t, os.WriteFile(
		filepath.Join(root, "sys/fs/cgroup/memory/docker/abc", "memory.limit_in_bytes"),
	...
	assert.Equal(t, "abc", usage.ContainerID)
	assert.Equal(t, 12.5, usage.CPUUsageSeconds)
	assert.Equal(t, 4096.0, *usage.MemoryUsageBytes)
	assert.Equal(t, 3072.0, *usage.MemoryWorkingSetBytes)
	assert.Nil(t, usage.CPULimitCores)
}
...
func TestRayServeRuntimeUsageProviderUsesRoutePrefixForEndpointIdentity(t *testing.T) {
	usageBytes := 1024.0
	provider := RayServeRuntimeUsageProvider{
		Dashboard: fakeRuntimeDashboard{
			nodes: []v1.NodeSummary{
		...
		NodeIP: "10.0.0.10",
		CGroupUsage: CGroupUsageReaderFunc(func(_ int) (ContainerRuntimeUsage, bool, error) {
			return ContainerRuntimeUsage{
				ContainerID:      "docker-abc",
				CPUUsageSeconds:  12.5,
				MemoryUsageBytes: &usageBytes,
	...
	require.NoError(t, err)
	require.Len(t, usages, 1)
	assert.Equal(t, "default_work_space", usages[0].Workspace)
	assert.Equal(t, "chat_model", usages[0].Endpoint)
}
...
func TestKubernetesCAdvisorRuntimeUsageProviderMapsPodMetricsToEndpointReplica(t *testing.T) {
	...
	client := fake.NewClientBuilder().
		WithScheme(scheme).
		WithIndex(&corev1.Pod{}, "spec.nodeName", runtimePodNodeNameIndex).
		WithObjects(pod).
		Build()
...
func runtimePodNodeNameIndex(object client.Object) []string {
