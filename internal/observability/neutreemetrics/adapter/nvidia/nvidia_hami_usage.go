...
func nvidiaVirtualizationUsages(evidence adapter.KubernetesEvidence) []model.EndpointReplicaGPUUsage {
	if !evidence.VirtualizationMonitorUp {
		return nil
	}

	usages := nvidiaHAMiEndpointReplicaUsagesFromMetrics(
		evidence.VirtualizationMonitorText,
		endpointPodIdentities(evidence.EndpointPods),
	)

	return internalEndpointReplicaAcceleratorUsages(usages)
}
