...
func TestNvidiaAdapterDiscoversHardware(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			accelerator := &accelerator{provider: nvidiaHardwareInfoProviderFunc(
				func(context.Context) ([]nvidiaHardwareInfo, error) {
...
func TestGPUHardwareInfosFromSnapshotRetainsImmutableDetails(t *testing.T) {
	...
	assert.Equal(t, nvidiaHardwareInfo{
		UUID:              "GPU-abc",
		Index:             "0",
		Product:           "NVIDIA L20",
		Architecture:      "Ada",
...
DCGM_FI_DEV_FB_TOTAL{gpu="0",UUID="GPU-abc",modelName="A100"} 81920`,
	...
	memoryUsage := requireNvidiaSample(t, actual.Samples, "neutree_endpoint_replica_accelerator_memory_used_bytes", map[string]string{
		"endpoint": "chat", "accelerator_uuid": "GPU-abc", "accelerator_index": "0", "product": "A100",
	})
	utilization := requireNvidiaSample(t, actual.Samples, "neutree_endpoint_replica_accelerator_utilization_ratio", map[string]string{
		"endpoint": "chat", "accelerator_uuid": "GPU-abc", "accelerator_index": "0", "product": "A100",
	})

	assert.Equal(t, memoryUsedBytes, memoryUsage.Value)
	assert.Equal(t, utilizationRatio, utilization.Value)
}
