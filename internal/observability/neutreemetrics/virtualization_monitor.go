...
// KubernetesVirtualizationMonitorCollector discovers the selected Profile's
...
func (c KubernetesVirtualizationMonitorCollector) Collect(ctx context.Context) (string, bool, error) {
	if c.Client == nil || c.NodeName == "" || c.Profile == nil {
		return "", false, nil
	}
	...
	options := []client.ListOption{
		...
		client.MatchingLabels(c.Profile.PodSelector),
	}
	if c.Profile.Namespace != "" {
		options = append(options, client.InNamespace(c.Profile.Namespace))
	}
...
func (c KubernetesVirtualizationMonitorCollector) scrape(ctx context.Context, podIP string) (string, error) {
	request, err := http.NewRequestWithContext(
		ctx,
		http.MethodGet,
		fmt.Sprintf("http://%s:%d%s", podIP, c.Profile.Port, normalizedTargetMetricsPath(c.Profile.MetricsPath)),
		nil,
	)
	if err != nil {
		return "", err
