...
func (k *Kong) generateRequestTransformPlugin(ep *v1.Endpoint, curRoute *kong.Route) *kong.Plugin {
	...
	return &kong.Plugin{
		Name:         pointy.String("request-transform"),
		InstanceName: pointy.String("neutree-request-transform-" + util.HashString(ep.Key())),
		Route:        curRoute,
		Config: map[string]interface{}{
...
func (k *Kong) syncPlugin(plugin *kong.Plugin) error {
	...
	if isResourceNotFoundError(err) {
		curPlugin, err = k.kongClient.Plugins.Create(context.Background(), plugin)
		if err != nil {
