...
func newCreateAdmissionRunnerWithOptions[T any](
	resolver createAdmissionChainResolver,
	resource admission.Resource[T],
	options CreateAdmissionRunnerOptions,
) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			switch {
			case options.ReadBodyResponse != nil:
				writeAdmissionRunnerError(c, options.ReadBodyResponse(err))
			case options.ReadBodyError != nil:
