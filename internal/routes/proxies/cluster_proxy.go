...
func validateClusterVersionCreate(s storage.Storage, provider ReleaseInfoProvider) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := validateClusterVersionCreateTarget(s, info, cluster.GetVersion(), clusterType); err != nil {
			c.JSON(http.StatusBadRequest, releaseInfoCreateValidationError(err))
			c.Abort()
...
func validateClusterVersionUpdate(s storage.Storage, providers ...ReleaseInfoProvider) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		clusterType := ""
		if input.New != nil && input.New.Spec != nil {
			clusterType = input.New.Spec.Type
		}

		if clusterType == "" && input.Current != nil && input.Current.Spec != nil {
			clusterType = input.Current.Spec.Type
		}

		if err := validateClusterVersionUpdateTarget(s, info, effectiveClusterVersionForUpdate(input.Current), input.New.GetVersion(), clusterType); err != nil {
...
func validateClusterVersionCreateTarget(s storage.Storage, info *v1.ReleaseInfo, version, clusterType string) error {
	if version == "" {
	...
	return requireExactClusterProfile(s, version, clusterType)
}
...
func validateClusterVersionUpdateTarget(
	...
	return requireExactClusterProfile(s, desiredVersion, clusterType)
}
...
func requireExactClusterProfile(s storage.Storage, version, clusterType string) error {
	if s == nil {
	...
	for _, profile := range profiles {
		if profile.GetName() == version && profile.GetClusterType() == clusterType {
			return nil
		}
	}

	return fmt.Errorf("cluster profile %s/%s not found", version, clusterType)
}
...
func RegisterClusterRoutes(group *gin.RouterGroup, middlewares []gin.HandlerFunc, deps *Dependencies) {
	...
	proxyGroup.POST("", versionCreateValidation, validation, handler)
	proxyGroup.PATCH("", validation, versionUpdateValidation, handler)
}
