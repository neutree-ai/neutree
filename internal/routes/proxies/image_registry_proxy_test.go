...
func TestRegisterImageRegistryRoutesForwardsCLIPatchWithAuthConfigOutsideAdmissionCandidate(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	var admittedCandidate v1.ImageRegistry
	require.NoError(t, registry.RegisterHook(imageRegistryAdmissionResource, admission.ValidateUpdate(
		admission.HookMeta{Name: "community.image-registry.capture-masked-update", Order: 900}, 91905,
	...
	body := strings.NewReader(`{
		...
		"spec":{
			"url":"https://registry.example.com:5000",
			"repository":"updated-repository",
			"authconfig":{"username":"updated-user","password":"updated-password"}
		}
	}`)
	req := httptest.NewRequest(http.MethodPatch, "/api/v1/image_registries?id=eq.118", body)
	req.Header.Set("Content-Type", "application/json")
	...
	require.Equal(t, v1.ImageRegistryAuthConfig{}, admittedCandidate.Spec.AuthConfig)
}
