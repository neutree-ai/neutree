...
// newRoutedEngine registers the real routes on an engine configured the way
// production configures it, with the permission middleware stubbed out. These
// tests are about the router, so what they must not do is call the handlers
// directly — that is exactly the layer where the bug lived: a request that never
...
// modelRegistryFixture wires storage and a registry client that record which
// model name the handler was given.
...
// Every model on the Hub is named "org/model". A client encodes the slash; the
// router has to keep it encoded long enough to match, and the handler has to
// receive it decoded.
...
// The regression this guards against is a framework-level 404: gin answering
// before any handler runs, with plain text rather than the API's JSON. Asserting
// only "not 404" would pass for the wrong reason if the route were removed, so
// this pins the shape of the old failure.
...
// A private registry's model names are validated to [a-z0-9._-], so none of them
// ever needs escaping. Their URLs must route exactly as before.
...
// The listing route sits at the same prefix and must not be shadowed by a model
// name that now spans what used to look like two segments.
...
// A literal slash is a different URL, not an encoded one, and there is no route
// with that shape. Left as a 404 on purpose: making the router guess how many
// trailing segments belong to a name would be ambiguous against /readme and
// /download.
