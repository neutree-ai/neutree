...
const (
	// readmeContentType is markdown, not HTML. The server hands back exactly what
	// the registry stores and renders nothing: a model card is content from
	// outside this system — a file on a public hub, or one written by anyone with
	// push rights — and turning it into HTML here would make every client that
	// displays the result run whatever it contained.
	readmeContentType = "text/markdown; charset=utf-8"
	// readmeTruncatedHeader marks a card that was longer than
	// model_registry.MaxReadmeBytes. Without it a client cannot tell a document
	// that ends abruptly from one that was written that way.
...
func respondReadmeError(c *gin.Context, modelName, version string, err error) {
	...
	if errors.Is(err, model_registry.ErrNotFound) {
		// Said in the model's own terms rather than the generic wording: "has no
		// README" is the fact, and it is not an error the user has to act on.
