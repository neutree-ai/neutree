...
func TestDerivePDConfig(t *testing.T) {
	ep := &v1.Endpoint{
	...
	cfg, err := DerivePDConfig(ep, "mooncake")
	if err != nil {
		t.Fatalf("DerivePDConfig: %v", err)
	}
	...
	for _, role := range cfg.Group.Roles {
		if role.PortsPerRank != 2 {
			t.Errorf("role %q PortsPerRank: got %d want 2", role.Name, role.PortsPerRank)
		}
	}
...
func TestDerivePDConfig_AllowsTemplateOwnedDefaultKVConnector(t *testing.T) {
	cfg, err := DerivePDConfig(&v1.Endpoint{
		Spec: &v1.EndpointSpec{
			Strategy: "pd",
			Roles: []v1.EndpointRoleSpec{
				{Name: "prefill"},
...
func TestValidatePD_Failures(t *testing.T) {
	tests := []struct {
		name    string
		ep      *v1.Endpoint
		wantSub string
