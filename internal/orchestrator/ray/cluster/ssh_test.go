...
func TestSSHClusterManager_UpCluster(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Setup
			mockExec := new(commandmocks.MockExecutor)
			mockExec.On("Execute", mock.Anything, mock.Anything, mock.Anything).Return(nil, nil).Once()
...
func TestStartNode_Success(t *testing.T) {
	...
	mockExec.On("Execute", mock.Anything, "ssh", mock.Anything).Run(func(args mock.Arguments) {
		...
		assert.Contains(t, strings.Join(cmd, " "), "nvidia-smi")
	}).Return(nil, nil).Once()
	mockExec.On("Execute", mock.Anything, "ssh", mock.Anything).Run(func(args mock.Arguments) {
		cmd := args.Get(2).([]string)
	...
	}{
		{
			...
			expectedConfig: &v1.RayClusterConfig{
				...
				HeadStartRayCommands: []string{
					"ray stop",
					`python /home/ray/start.py --head --port=6379 --metrics-export-port=54311 --disable-usage-stats --object-manager-port=8076 --autoscaling-config=~/ray_bootstrap_config.yaml --dashboard-host=0.0.0.0 --labels='{"neutree.ai/neutree-serving-version":"v1.0.0"}'`,
				},
