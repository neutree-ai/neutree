...
//go:embed schemas/llama_cpp_v0.3.6_engine_schema.json
...
// GetLlamaCppDefaultEngineSchema returns the parsed JSON schema for Llama.cpp V0.3.6 engine
func GetLlamaCppDefaultEngineSchema() (map[string]interface{}, error) {
	...
	if err := json.Unmarshal(llamaCppDefaultEngineSchema, &schema); err != nil {
		return nil, fmt.Errorf("failed to parse Llama.cpp v0.3.6 engine schema: %w", err)
	}
...
var EngineSchemas = map[string]func() (map[string]interface{}, error){
	...
	"llama-cpp-v0.3.6": GetLlamaCppDefaultEngineSchema,
}
