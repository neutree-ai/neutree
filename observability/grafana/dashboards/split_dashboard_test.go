...
func TestSplitDashboardsAreValidAndDoNotUseRemovedMetrics(t *testing.T) {
	for _, file := range splitDashboardFiles {
		t.Run(file, func(t *testing.T) {
			...
			assert.NotContains(t, text, "or vector(0)")
		})
	}
}
...
func TestEndpointCacheCapacityUsesEngineTokenCapacity(t *testing.T) {
	...
	assertNestedString(t, panel.FieldConfig, "locale", "defaults", "unit")
	assertNestedFloat(t, panel.FieldConfig, 0, "defaults", "decimals")
}
...
func TestEndpointGPUAllocationDetailTablesMatchClusterDisplay(t *testing.T) {
	...
	for _, file := range []string{
		...
		t.Run(file, func(t *testing.T) {
			...
			expectedIndexByName["Value #D"] = float64(5)
			expectedIndexByName["product"] = float64(6)
			expectedIndexByName["architecture"] = float64(7)
			expectedIndexByName["cuda_capability"] = float64(8)
			expectedIndexByName["driver_version"] = float64(9)
			expectedIndexByName["cuda_driver_version"] = float64(10)
			expectedIndexByName["Value #E"] = float64(11)
			expectedIndexByName["nvlink"] = float64(12)
			expectedIndexByName["nvswitch"] = float64(13)
			expectedIndexByName["pcie_bus_id"] = float64(14)
			expectedIndexByName["pcie_generation"] = float64(15)
			expectedIndexByName["pcie_width"] = float64(16)
			expectedIndexByName["numa_node"] = float64(17)
...
func TestEndpointRuntimePanelsOnlyShowBackendReplicas(t *testing.T) {
	for _, title := range []string{"CPU cores", "Memory working set", "CPU usage trend", "Memory usage trend"} {
		t.Run(title, func(t *testing.T) {
			expressions := strings.Join(
				dashboardPanelExpressions(t, "neutree_endpoint_overview_embed_dashboard.json", title),
				"\n",
			)
			assert.Contains(t, expressions, `container="Backend"`)
		})
	}

	for _, title := range []string{"CPU usage trend", "Memory usage trend"} {
		t.Run(title, func(t *testing.T) {
			panel := dashboardPanel(t, "neutree_endpoint_overview_embed_dashboard.json", title)
			require.Len(t, panel.Targets, 1)
			assert.Equal(t, "Replica-{{replica}}", panel.Targets[0].LegendFormat)
			assert.Contains(t, panel.Targets[0].Expr, "sum by (replica)")
			assert.NotContains(t, panel.Targets[0].Expr, "deployment")
		})
	}
}
...
func assertGPUAllocationDetailTable(t *testing.T, file string, endpointScoped bool, withRuntimeModel bool) {
	...
	if withRuntimeModel {
		...
		assert.Contains(t, panel.Targets[5].Expr, "vllm:engine_sleep_state")
		assert.Contains(t, panel.Targets[5].Expr, `model_name=~".+"`)
		assert.Contains(t, panel.Targets[5].Expr, `served_model_name=~".+"`)
		assert.Contains(t, panel.Targets[5].Expr, `label_replace`)
		assert.Contains(t, panel.Targets[5].Expr, `label_set`)
		assert.Contains(t, panel.Targets[5].Expr, "unless on (replica)")
		assert.Contains(t, panel.Targets[5].Expr, `model_replica`)
		assert.Contains(t, panel.Targets[5].Expr, `application`)
		assert.Contains(t, panel.Targets[5].Expr, "group_left(model_name)")
	}
	...
	assertNestedString(t, organize, "PCIe Transfer", "renameByName", "Value #D")
	assertNestedString(t, organize, "Temperature", "renameByName", "Value #E")
	...
	assertFieldOverrideUnit(t, panel, "PCIe Transfer", "Bps")
...
func assertGPUAllocationDetailColumnOrder(
	...
	if endpointScoped && withRuntimeModel {
		...
		expectedPrefix["Value #D"] = 5
		expectedPrefix["product"] = 6
		expectedPrefix["architecture"] = 7
		expectedPrefix["cuda_capability"] = 8
		expectedPrefix["driver_version"] = 9
		expectedPrefix["cuda_driver_version"] = 10
		expectedPrefix["Value #E"] = 11
		expectedPrefix["nvlink"] = 12
		expectedPrefix["nvswitch"] = 13
		expectedPrefix["pcie_bus_id"] = 14
		expectedPrefix["pcie_generation"] = 15
		expectedPrefix["pcie_width"] = 16
		expectedPrefix["numa_node"] = 17
	}
