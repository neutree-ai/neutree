...
- Ascend accelerator type 由企业版拥有。历史 Enterprise/runtime 输入可见 `npu-ascend310p`、`npu-ascend910b` 等 variant；本设计将 `npu` 定义为新的 canonical family，社区版只把任意 accelerator type 当作 opaque string 透传。
...
- Roadmap 1 交付进程内 extension contract、Enterprise 注册、feature gate、accelerator quota 语义，以及可独立单测的 Ascend plugin 实现。它不验收静态/Kubernetes 集群编排、runtime image、endpoint 部署或 engine import；这些集成路径分别由 Roadmap 2-3 验收。
...
旧 plugin server 的 `Run()` 和 `Register()` 不迁移到主路径。新 `ascend.Plugin` 直接实现 public `accelerator.Plugin`：`Type()` 返回 generic internal plugin type，`Handle()` 返回自身，`DetectStaticNodeAccelerator()` 复用 node detection 结果并填充标准 response，`Ping()` 对进程内 plugin 返回 nil。`GetAcceleratorProfile()` 返回 `AcceleratorType=npu` 的 family-level capability，不能为 310P/910B 假定 image suffix；Roadmap 2 必须使用已解析的 runtime profile 选择运行时配置。`MetricsExporter` 保持 nil，本设计不定义监控能力。
...
- 本 Roadmap 不修改 UI/API 文案；后续 UI/API 如需展示该配额，应优先使用 “Accelerator” 或“加速卡/加速器配额”文案，底层兼容字段仍叫 `GPU`。
...
#### Runtime profile 的来源与持久化

`StaticNodeAcceleratorStatus` 继续只记录 opaque accelerator family 和设备 product，不增加 `runtime_profile` 字段。静态集群 reconcile 在所有节点 discovery 完成后，按设备 `ProductModel` 做一次集群级归一：`HUAWEI_Ascend310P -> npu-ascend310p`、`HUAWEI_Ascend910B -> npu-ascend910b`。该映射由 Enterprise Ascend plugin 提供，社区版不出现 Ascend 常量。

Roadmap 2 在 `ClusterStatus` 增加通用、opaque 的 `AcceleratorRuntimeProfile *string`。当 `AcceleratorType=npu` 时，reconcile 必须在同一次 status write 中写入唯一的已解析 profile；它是后续启动、更新、pre-pull 和离线包选择的唯一 source of truth，不能从 type 反推。对 NVIDIA/AMD 等不需要 variant 的 accelerator，该字段为空并保持既有行为。

新 static-node flow 的 `StaticNodeCluster` 由独立 controller 消费，不能直接读取 parent `ClusterStatus`。因此 parent reconciler 必须把已解析的 opaque profile 复制到 `StaticNodeClusterSpec.RuntimeProfile`（控制面派生字段，用户 API 不可直接设置）；planner 仅消费该字段取得 profile-aware runtime config。parent status 仍是跨 reconcile/legacy SSH upgrade 的 source of truth，child spec 只是本次 static-node desired state 的传播副本。

- 检测到一个 profile 时，接受并持久化它。
- 节点没有可映射 product、用户指定 profile 与检测结果不一致、或节点集合产生多个 profile 时，cluster 进入失败状态并返回包含节点和 profile 的明确错误。
- 后续 reconcile、控制面重启和 SSH upgrade pre-pull 都只读取已持久化的 profile；它不存在时必须重新 discovery，不得对 `npu` 选择默认 suffix。
- staticcluster planner 和 manager 增加通用的 profile-aware runtime lookup，例如 `GetRuntimeConfig(ctx, acceleratorType, runtimeProfile)` 与 `GetImageSuffix(acceleratorType, runtimeProfile)`。现有只接受 type 的 lookup 仅用于无 variant accelerator，不能用于 Ascend static cluster。
...
runtime image suffix 是静态 Ray/SSH 集群的运行时 profile，不等价于对外 accelerator type。对外 accelerator type 仍由企业版 plugin 拥有并作为 opaque string 透传；310P/910B 的差异通过已持久化的 runtime profile、product、runtime image suffix 和 image label 体现。
...
cluster versions API 不能把 `npu-ascend310p` 和 `npu-ascend910b` 当作 accelerator type 来过滤。Roadmap 2 的选定方案是在 `GET /clusters/available_versions` 增加可选的通用 query 参数 `runtime_profile` 和 `accelerator_product`：服务端在读取 registry image labels 时，与已有 `accelerator_type` 一起按 AND 语义过滤 `neutree.ai/runtime-profile`、`neutree.ai/accelerator-product`。省略两个参数保持现有仅按 type 的版本字符串响应；给定参数时仍只返回匹配镜像的去重版本字符串，非法或冲突参数返回 `400`。不采用 Enterprise 调用方二次过滤，因为当前响应不包含 labels。首期不要求社区版新增 Ascend accelerator type 常量。
...
- 可用版本查询：`GET /clusters/available_versions` 对 SSH cluster 查询 `neutree/neutree-serve`，以 `accelerator_type=npu` 加已持久化的 `runtime_profile` 和对应 product 参数过滤 image labels。
- SSH cluster 启动：`cluster.Spec.Version` 继续保存纯版本，例如 `v1.0.1`；planner 以 `ClusterStatus.AcceleratorRuntimeProfile` 取得 `ImageSuffix`，得到 `v1.0.1-npu-ascend910b`。
- SSH cluster 升级 pre-pull：使用 `ClusterStatus.AcceleratorRuntimeProfile` 调用 profile-aware suffix lookup，不能从 `ClusterStatus.AcceleratorType` 推断或退回默认 `neutree-serve:<version>`。
...
- 如果用户只配置 NPU family 而未提供 product/runtime profile，静态集群应执行节点探测；无法唯一确定 310P/910B 时返回明确错误。用户如提供 profile，必须与 discovery 的唯一结果一致。
- license 和 quota 统计按 accelerator family 归并，runtime variant 不新增独立 license resource type。

同一静态 Ray/SSH 集群内 310P 和 910B 混合运行不作为当前版本目标。发现多个 profile 时本版本明确拒绝该 cluster；本版本只接受能归一为单一 runtime profile 的静态集群，并为其选择正确基础镜像、runtime config 和 image list。后续若支持混合集群，需要单独设计节点级 runtime profile、按节点 pre-pull、Ray resource 聚合和 endpoint 调度约束。
...
- 使用 Roadmap 1 Ascend plugin 接入静态节点探测和 runtime config。
- 通用 `ClusterStatus.AcceleratorRuntimeProfile`、profile-aware runtime/suffix lookup，以及 310P/910B/mixed-profile 的 discovery、持久化和升级回归测试。
- 静态节点 `Dockerfile.npu` 和 310P/910B build targets。
- 静态节点 310P/910B cluster image tag、label 和版本过滤规则。
- `available_versions` 的通用 `runtime_profile` / `accelerator_product` server-side 过滤、参数校验和兼容响应测试。
- Ascend 310P/910B 离线 cluster package image lists。
- 将 Roadmap 1 的 Ray/Kubernetes converter、parser 和 310P/910B product mapping 接入 cluster reconcile 与 endpoint resource validation。
...
- 静态 Ray/SSH cluster 在控制面重启和升级 pre-pull 后仍使用已持久化的 310P 或 910B profile；混合 profile cluster 被拒绝。
