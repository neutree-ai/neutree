...
- 引入 `StaticCluster` 和 `StaticNode` 两个独立资源，分别承担集群级编排和节点级收敛。
- 将节点上的常驻组件抽象为 `NodeWorker`，第一版作为 `StaticNode` 的嵌入式工作负载。
- 静态集群 V1 不再依赖 `ray up` 启动 head node，head 和 worker 都由 `StaticNodeReconciler` 收敛。
- 每个 `StaticNode` 上运行 daemonset-like 的 `neutree-metrics` worker。
...
- 引入 `spec.warm` 语义，由 `StaticNodeReconciler` 负责预热节点资源。V1 仅实现 image warm。
...
- V1 不把 `NodeWorker` 做成独立资源。
- V1 不实现 vmagent HA。
- V1 不实现节点本地 agent。
- V1 不实现通用 worker 插件系统。
...
`StaticCluster` 和 `StaticNode` 都是一等资源，拥有独立的 `spec`、`status`、`generation`、`conditions` 和 reconcile queue。

`NodeWorker` 第一版嵌入 `StaticNode.spec` / `StaticNode.status`。

## StaticCluster

`StaticCluster` 表示一个静态 Ray 集群的集群级期望状态。
...
- 保存单节点期望状态。
- 保存该节点应该运行的 `NodeWorker` 列表。
- 保存节点 warm 状态和 worker 健康状态。
- 独立执行 SSH、Docker、image pull、worker reconcile。

示例结构：

```yaml
spec:
  cluster: static-cluster-a
  ip: 10.0.0.10
  role: head
...
`NodeWorker` 是 Neutree 在节点上管理的常驻工作负载。
...
`StaticClusterReconciler` 负责生成每个节点的 `spec.warm`。
...
`StaticClusterReconciler` 聚合 `StaticNode.status.warm`，required warm 全部 ready 后才能进入 stop/start 阶段。

## Reconcile 分层

### StaticClusterReconciler
...
6. 计算 StaticCluster.status.phase。
...
3. 执行 docker login。
4. 收敛 spec.warm。
5. 根据 workers 计算 config hash。
6. 创建、重建或检查 NodeWorker。
7. 执行 worker health check。
...
| provider 查询/创建 head，并打 launch/status/name tags | Ray `get_or_create_head_node` | 不必要 | `StaticCluster` 创建 `StaticNode(head)`；状态下沉到 `StaticNode.status` |
| launch hash 判断是否重建 head | Ray `hash_launch_conf` + provider tags | 必要但换实现 | 用 `NodeWorker.config_hash` / `observed_hash` 表达，不依赖 provider tags |
| runtime hash / file_mounts contents hash | Ray `hash_runtime_conf` | 必要但换实现 | 用 worker config hash 覆盖 command、image、run options、config_files 内容 |
| 等 SSH ready，状态从 waiting-for-ssh 到 syncing/setting-up/up-to-date | Ray `NodeUpdater.do_update` | 必要 | `StaticNode.status.conditions` 表达 SSHReady、FilesReady、WorkersReady |
...
| 根据 accelerator 修改 Docker image suffix、runtime、env、options | `buildAcceleratorDockerConfig` | 必要 | 迁移为 `ray-head` / `ray-worker` worker 的 Docker config 生成逻辑，继续通过 AcceleratorManager 获取 runtime config |
...
| `--no-restart` 控制是否重启 head | `upCluster(restart)` | 不必要 | 由 `config_hash` / `observed_hash` 控制是否重建 worker；需要重启时显式 stop/start |
| Ray local provider 启动 head Docker container | Ray autoscaler 内部 | 不必要 | 由 `DockerCommandRunner.RunInit` 或新的 NodeWorker runtime 负责启动容器 |
...
| container image / bind mounts drift 检查，不匹配时 stop/recreate | Ray DockerCommandRunner | 必要 | Neutree DockerCommandRunner 需要补齐 image/mount drift 检查，或者由 worker config hash 驱动 recreate |
...
| dashboard API 健康检查和 alive head raylet 检查 | `checkHeadNodeHealth` / `initHeadNode` | 必要 | 保留为 `ray-head` worker health check 和 StaticCluster verify |
| `ray down` 停整个集群 | `downCluster` | 不必要 | StaticCluster 推进 StopCluster；StaticNode 分别停止 `ray-worker`、`ray-head` |
| `NodeProvisionStatus` 聚合在 Cluster status | `reconcileWorkerNode` / `setNodePrivisionStatus` | 不必要 | 下沉到 `StaticNode.status.workers[*]` 和 `StaticNode.status.phase` |
...
- head 节点仍需要补 Ray DockerCommandRunner 的 container drift 检查，或者由 Neutree worker config hash 显式驱动 recreate。
- Ray autoscaler local provider、local cluster state、`ray up --no-restart`、`ray down` 不应继续作为 V1 静态集群核心语义。
- 远程写配置仍然需要，但目标是 vmagent、`neutree-metrics`、NodeWorker env/config，不是 Ray autoscaler bootstrap。

替代关系：

```text
ray up head
  -> StaticNode(head).workers[ray-head]

ray up generated config files
  -> V1 drops Ray bootstrap config files
  -> StaticNode worker config_files only for Neutree-managed components

ray down
  -> StaticCluster phase StopCluster
  -> StaticNode worker stop

ray up --no-restart
  -> config_hash / observed_hash
  -> only restart changed workers
```
...
5. Write Neutree-managed component config files, if the worker has config_files.
...
11. Update StaticNode.status.workers[ray-head].
```
...
3. Run host initialization commands.
4. Warm required images.
5. Write Neutree-managed config files, if the worker has config_files.
6. Start or recreate Ray runtime container.
7. Run container initialization commands.
8. Run ray stop.
9. Run role-specific Ray start command.
10. Run role-specific health check.
11. Update StaticNode.status.workers[*].
```
...
| failure impact | `StaticCluster` not ready | `StaticCluster` degraded，不阻塞其他节点 reconcile |
...
head node 额外运行的 `vmagent` 不是 `ray-head` 启动差异，而是 head 节点上多一个 `metrics-agent` NodeWorker。

结论：

- V1 不应保留两套 Ray 启动流程。
- 应实现一个通用 `RayRuntimeWorkerReconciler`，根据 `role=head|worker` 生成不同 Ray start command 和 health check。
...
- `WriteFileIfChanged` 先比较远端 sha256，不变则不写文件，不触发 worker restart。
...
```text
StaticClusterReconciler renders vmagent scrape config
  -> StaticNode(head).workers[vmagent].config_files
  -> StaticNodeReconciler
...
```text
StaticClusterReconciler renders per-node neutree-metrics config
  -> StaticNode(node).workers[neutree-metrics].config_files
...
- 每个 `StaticNode` 运行一个 `neutree-metrics` worker。
- head node 运行一个 `vmagent` worker。
...
accelerator_profile:
  ...
  metrics:
    exporter:
      kind: dcgm-exporter
      worker_type: accelerator-exporter
...
`StaticClusterReconciler` 根据节点的 `accelerator_type` 和 accelerator profile 生成：

- `StaticNode.spec.workers` 中的 exporter worker。
...
```text
EndpointReconciler / StaticClusterReconciler / StaticNodeReconciler
  -> AcceleratorManager
...
accelerator exporter 镜像、端口和运行参数通过 plugin profile 下发。`StaticClusterReconciler` 负责把 profile 转成 `NodeWorker` 和组件配置。`neutree-metrics` 根据 `accelerator_type` / `exporter_kind` 执行 canonical metrics mapping。
...
- vmagent worker
- neutree-metrics worker
- metrics scrape config

这些属于 `StaticCluster` / `StaticNode`。
...
新架构下，`StaticCluster` 推进阶段，`StaticNode` 执行节点级动作。

### Warm

`StaticClusterReconciler` 更新每个 `StaticNode.spec.warm`。
...
- head Ray 不 ready：`StaticCluster` not ready。
...
- 单个 worker node 不 ready：`StaticCluster` degraded，不阻塞其他节点。

## V1 任务拆分

1. 定义 `StaticCluster` / `StaticNode` 资源模型。
2. 新增 optional `AcceleratorProfile`。
3. 定义 `NodeWorker` 嵌入模型。
...
10. 实现 head-local vmagent worker 和 scrape config 下发。
...
第一版不新增数据库 schema、RLS、trigger 或 migration 时，不需要新增 DB test。后续落地独立 `StaticCluster` / `StaticNode` 持久化资源时，需要补充：

- `StaticCluster` 与 `StaticNode` CRUD、workspace 隔离、权限校验。
- `StaticNode.status.workers` 局部更新不会阻塞或覆盖 `StaticCluster.status`。
- warm / worker status 字段的 JSON schema 和查询过滤。
...
- `StaticCluster` 更新不会被单个节点的 SSH、image pull、worker 启动阻塞。
- `StaticNode` 可独立 reconcile，并记录每个 worker 的状态。
- head node 启动不再调用 `ray up`，而是通过 `StaticNode` 的 `ray-head` worker 收敛。
- command_runner 支持远程写文件，vmagent 和 `neutree-metrics` 配置可通过 `config_files` 下发。
- 配置文件未变化时不重启对应 worker。
...
- 修改 `StaticCluster.spec.version` 后，可执行 warm、stop、start、verify 流程。
