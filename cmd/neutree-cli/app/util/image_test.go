...
func TestReplaceImageRegistry(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "mirror registry with https scheme",
			imageURL:       "docker.io/library/nginx:latest",
			mirrorRegistry: "https://mirror.example.com",
			want:           "mirror.example.com/library/nginx:latest",
			wantErr:        false,
		},
		{
			name:           "mirror registry with https scheme and port",
			imageURL:       "docker.io/library/nginx:latest",
			mirrorRegistry: "https://mirror.example.com:5000",
