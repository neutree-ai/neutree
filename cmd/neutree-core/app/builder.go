...
type Builder struct {
	...
	initControllers map[string]ControllerFactory
...
func NewBuilder() *Builder {
	b := &Builder{
		...
		initControllers: make(map[string]ControllerFactory),
	...
	for name, factory := range defaultControllers {
		b.initControllers[name] = factory
	}
...
func (b *Builder) WithGlobalBeforeReconcileHook(hook controllers.HookFunc) *Builder {
	for name := range b.initControllers {
...
func (b *Builder) Build() (*App, error) {
	...
	for name, factory := range b.initControllers {
		...
		b.controllers[name] = ctrl
	}

	return NewApp(b.config, b.controllers), nil
}
