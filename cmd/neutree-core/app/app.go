...
func (a *App) currentControlPlaneBaseline() (baselineResolution, error) {
	if a == nil || a.config == nil || a.config.Storage == nil {
		return baselineResolution{}, fmt.Errorf("release profile storage is required")
	}
	...
	if _, normalizeErr := releaseprofile.NormalizeControlPlaneRelease(baseline); normalizeErr != nil {
		return baselineResolution{}, fmt.Errorf("current release profile catalog baseline %q must be an exact release identity", baseline)
	}
