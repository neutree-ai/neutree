...
type EngineVersion struct {
	...
	// NativeRuntime opts SSH/static Ray deployments into the generic passthrough
	// runner. The runner runs the engine server as a direct child of its Ray
	// Actor inside the Ray-injected engine image.
	NativeRuntime *NativeEngineRuntime `json:"native_runtime,omitempty" yaml:"native_runtime,omitempty"`
}

// NativeEngineRuntime describes the process and HTTP contract for the generic
// static-cluster passthrough runner. Image selection remains in
// EngineVersion.Images so accelerator-specific variants use the existing API.
type NativeEngineRuntime struct {
	// Command starts the engine server in the Ray-injected engine image.
	Command []string `json:"command,omitempty" yaml:"command,omitempty"`
	// RunOptions are engine-container options required by the engine, such as
	// vLLM's shared IPC namespace. Accelerator and model-mount options remain
	// generated by the cluster orchestrator.
	RunOptions  []string `json:"run_options,omitempty" yaml:"run_options,omitempty"`
	// Protocol is retained for existing Engine CRD round-trips. The generic
	// passthrough runner does not interpret it.
	...
	// Port is retained for existing Engine CRD round-trips. Direct runners use
	// a node-local allocated port instead.
