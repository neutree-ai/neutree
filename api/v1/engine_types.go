...
type EngineVersion struct {
	...
	// NativeRuntime opts SSH/static Ray deployments into the cluster-resident
	// native engine runner. The engine image remains unmodified; the runner uses
	// this launch contract to start and proxy the image as a Docker sibling.
	NativeRuntime *NativeEngineRuntime `json:"native_runtime,omitempty" yaml:"native_runtime,omitempty"`
}

// NativeEngineRuntime describes the stable, engine-owned network contract the
// generic static-cluster runner needs to start an unmodified engine image.
// Image selection remains in EngineVersion.Images so accelerator-specific
// variants continue to use the existing API.
type NativeEngineRuntime struct {
	// Command optionally prefixes arguments after the image reference. Leave it
	// empty when the raw image already provides the desired ENTRYPOINT, as the
	// vLLM OpenAI image does.
	Command []string `json:"command,omitempty" yaml:"command,omitempty"`
	// RunOptions are engine-specific Docker options required by the raw image,
	// such as vLLM's shared IPC namespace. Accelerator and model-mount options
	// remain generated by the cluster orchestrator.
