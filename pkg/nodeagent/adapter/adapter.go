...
// VirtualizationMonitorEvidence is one raw monitor observation. Up separates
// an unavailable endpoint from a successful scrape that happened to contain
// no samples, while Text remains untouched for the selected adapter to parse.
type VirtualizationMonitorEvidence struct {
	Text string
	Up   bool
}
...
type KubernetesEvidence struct {
	...
	// VirtualizationMonitor is unparsed exposition from the local monitor
...
func (e KubernetesEvidence) Clone() KubernetesEvidence {
	result := KubernetesEvidence{
		...
		VirtualizationMonitor: e.VirtualizationMonitor,
	}
...
// CloneStringMap returns a copy that callers may safely mutate.
