...
func SortedServeApplicationNames(response *dashboard.RayServeApplicationsResponse) []string {
	if response == nil {
		return nil
	}

	names := make([]string, 0, len(response.Applications))
	for name := range response.Applications {
...
func ApplicationIdentity(appName string, status dashboard.RayServeApplicationStatus) (string, string) {
