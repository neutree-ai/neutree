...
| V19 | User-supplied `Role.Env` / `Role.Variables` win over platform-derived values on key collision; non-conflicting platform values still propagate | set `roles[].env.VLLM_LOGGING_LEVEL=DEBUG` and `roles[].variables.max_num_seqs=128` in EndpointSpec; verify both reach the actor process. Platform-controlled keys (currently only `VLLM_NIXL_SIDE_CHANNEL_HOST`/`PORT` and `kv_transfer_config`/`distributed_executor_backend`): set a collision like `roles[].env.VLLM_NIXL_SIDE_CHANNEL_PORT=99999`, expect `[pd_collocated][env_vars] user overriding platform-controlled key` warning + actor binds 99999. `UCX_TLS` is NOT platform-controlled — engine/OS default applies unless user sets it. | | |
